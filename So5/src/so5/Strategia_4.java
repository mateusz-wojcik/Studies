...
public class Strategia_4 {
	Procesor[] procesor;
	
	public Strategia_4(Procesor[] proc){
		this.procesor = proc;
	}
	
	public void str4(int czas, int z, int p, int progR){ //z - ile procesorw pytamy p-prog obciazenia
		Random r = new Random(); 
				...
				for(int k=0; k<procesor[j].proc.size(); k++){         //sprawdzamy zgloszenia (ktore trwaja)
					...
					if(k<procesor[j].proc.size() && procesor[j].proc.get(k).pojawienie==i){        //sprawdzamy czy pojawil sie nowy proces
						...
						if(procesor[j].obciazenie>p){
				    		...
				    		while(!czy){
				    			/*for(int m=0; m<procesor.length; m++){
				    				if(procesor[m].obciazenie<p) licznik++;
				    			}*/
				    			licznik++;
